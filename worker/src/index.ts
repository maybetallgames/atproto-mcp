...
type WorkerEnv = {
  ...
  GITHUB_CLIENT_ID: string; GITHUB_CLIENT_SECRET: string; GITHUB_ALLOWED_LOGIN: string;
...
async function authorize(request: Request, env: WorkerEnv): Promise<Response> {
  if (request.method === "GET") {
    ...
    response.headers.append("Set-Cookie", setCookie("oauth_consent", await signedPayload({ oauthRequest, csrf, githubState }, env.COOKIE_ENCRYPTION_KEY)));
    return response;
  }
  if (request.method !== "POST") return new Response("Method not allowed", { status: 405 }); const form = await request.formData(); const pending = await verifiedPayload<{ oauthRequest: AuthRequest; csrf: string; githubState: string }>(cookies(request).oauth_consent, env.COOKIE_ENCRYPTION_KEY); if (!pending) return new Response("Authorization session is invalid or expired", { status: 400 }); if (form.get("csrf") !== pending.csrf) return new Response("CSRF validation failed", { status: 403 }); if (form.get("decision") !== "approve") { const denied = new URL(pending.oauthRequest.redirectUri); denied.searchParams.set("error", "access_denied"); denied.searchParams.set("state", pending.oauthRequest.state); return Response.redirect(denied.toString(), 302); } const state = pending.githubState; const redirect = new URL("https://github.com/login/oauth/authorize"); redirect.searchParams.set("client_id", env.GITHUB_CLIENT_ID); redirect.searchParams.set("redirect_uri", `${new URL(request.url).origin}/callback`); redirect.searchParams.set("scope", "read:user"); redirect.searchParams.set("state", state); const response = new Response(null, { status: 302, headers: { Location: redirect.toString() } }); response.headers.append("Set-Cookie", setCookie("github_state", await signedPayload({ state, oauthRequest: pending.oauthRequest }, env.COOKIE_ENCRYPTION_KEY))); return response;
}
async function callback(request: Request, env: WorkerEnv): Promise<Response> { const url = new URL(request.url), code = url.searchParams.get("code"), state = url.searchParams.get("state"), pending = await verifiedPayload<{ state: string; oauthRequest: AuthRequest }>(cookies(request).github_state, env.COOKIE_ENCRYPTION_KEY); if (!code || !state || !pending || state !== pending.state) return new Response("GitHub OAuth state validation failed", { status: 400 }); const tokenResponse = await fetch("https://github.com/login/oauth/access_token", { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/json", "User-Agent": "bluesky-community-manager" }, body: JSON.stringify({ client_id: env.GITHUB_CLIENT_ID, client_secret: env.GITHUB_CLIENT_SECRET, code, redirect_uri: `${url.origin}/callback` }) }); const tokenData = await tokenResponse.json() as { access_token?: string; error_description?: string }; if (!tokenResponse.ok || !tokenData.access_token) return new Response(`GitHub token exchange failed: ${tokenData.error_description ?? tokenResponse.status}`, { status: 502 }); const userResponse = await fetch("https://api.github.com/user", { headers: { Authorization: `Bearer ${tokenData.access_token}`, Accept: "application/vnd.github+json", "User-Agent": "bluesky-community-manager", "X-GitHub-Api-Version": "2022-11-28" } }); const user = await userResponse.json() as { id?: number; login?: string; name?: string }; if (!userResponse.ok || !user.id || !user.login) return new Response("Could not verify GitHub identity", { status: 502 }); if (user.login.toLowerCase() !== env.GITHUB_ALLOWED_LOGIN.toLowerCase()) return new Response("This GitHub account is not authorized for this Bluesky manager", { status: 403 }); const completed = await env.OAUTH_PROVIDER.completeAuthorization({ request: pending.oauthRequest, userId: `github:${user.id}`, metadata: { label: user.login }, scope: pending.oauthRequest.scope, props: { githubId: user.id, githubLogin: user.login } }); const response = new Response(null, { status: 302, headers: { Location: completed.redirectTo } }); response.headers.append("Set-Cookie", setCookie("github_state", "", 0)); return response; }

const apiHandler = { async fetch(request: Request, env: WorkerEnv): Promise<Response> { return mcp(request, env); } };
const defaultHandler = { async fetch(request: Request, env: WorkerEnv): Promise<Response> { const path = new URL(request.url).pathname; if (path === "/health") return json({ ok: true, service: "bluesky-community-manager", authentication: "oauth" }); if (path === "/authorize") return authorize(request, env); if (path === "/callback") return callback(request, env); if (path === "/") return new Response("Bluesky Community Manager MCP server", { headers: { "Content-Type": "text/plain; charset=utf-8" } }); return new Response("Not found", { status: 404 }); } };
