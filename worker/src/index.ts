...
const tools = [
  ...
  },
  {
    ...
    description:
      'Read and summarize new development facts for the social layer. Does not publish or advance the checkpoint.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    annotations: { readOnlyHint: true },
  },
  {
    name: 'record_devlog_post',
    description:
      'After a successful Bluesky devlog post, record its URI and commit references in Cloudflare KV.',
    inputSchema: {
      type: 'object',
      properties: {
        uri: { type: 'string' },
        commitReferences: { type: 'array', items: { type: 'string' }, minItems: 1 },
      },
      required: ['uri', 'commitReferences'],
      additionalProperties: false,
    },
  ...
  },
  {
    ...
    description:
      'Get new Bluesky likes, followers, replies, mentions, quotes, and reposts. Uses a saved checkpoint when since is omitted. Set advanceCheckpoint only after successfully processing the response.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Read a Bluesky post and its surrounding thread before deciding whether or how to reply.',
    inputSchema: {
      type: 'object',
      properties: {
        uri: { type: 'string', pattern: '^at://' },
        depth: { type: 'integer', minimum: 0, maximum: 20, default: 6 },
        parentHeight: { type: 'integer', minimum: 0, maximum: 100, default: 20 },
      },
      required: ['uri'],
      additionalProperties: false,
    },
  ...
  },
  {
    name: 'reply_to_post',
    description:
      "Publish a Bluesky reply. Intended for comments or mentions on this account's own posts after inspecting the thread.",
    inputSchema: {
      type: 'object',
      properties: {
        text: { type: 'string', minLength: 1, maxLength: 3000 },
        root: { type: 'string', pattern: '^at://' },
        parent: { type: 'string', pattern: '^at://' },
        langs: { type: 'array', items: { type: 'string' } },
      },
      required: ['text', 'root', 'parent'],
      additionalProperties: false,
    },
    annotations: {
      readOnlyHint: false,
  ...
  },
  {
    name: 'create_post',
    description:
      'Publish a new standalone Bluesky post. Accepts files attached directly in ChatGPT through mediaFiles, plus explicit HTTPS URLs/base64 inputs. Supports up to four images or one video.',
    inputSchema: {
      type: 'object',
      properties: {
        text: { type: 'string', minLength: 0, maxLength: 3000 },
        langs: { type: 'array', items: { type: 'string' } },
        images: {
          type: 'array',
          maxItems: 4,
          items: {
            ...
            properties: {
              ...
              alt: { type: 'string', maxLength: 2000, default: '' },
        ...
        mediaFiles: {
          ...
          description:
            'Files attached directly in ChatGPT. Each file is supplied by ChatGPT with a temporary HTTPS download_url and file_id.',
        },
      },
      additionalProperties: false,
    },
  ...
  },
  {
    ...
    description:
      'Publish a standalone Bluesky post from files attached in ChatGPT. Use this tool for an attached video or up to four attached images.',
    inputSchema: {
      type: 'object',
      properties: {
        text: { type: 'string', minLength: 0, maxLength: 3000 },
        ...
        mediaFiles: {
          ...
          description: 'Files attached by ChatGPT. One video or up to four still images.',
        },
      },
      required: ['mediaFiles'],
      additionalProperties: false,
  ...
  },
  {
    ...
    title: 'Post attached media to Bluesky',
    description:
      'Publish a standalone Bluesky post using files attached in ChatGPT. This dedicated file-input tool is preferred whenever the user supplies an image or video attachment.',
    inputSchema: {
      type: 'object',
      properties: {
        text: { type: 'string', minLength: 0, maxLength: 3000 },
        ...
        mediaFiles: {
          ...
          description:
            'ChatGPT file inputs. ChatGPT supplies download_url and file_id automatically; mime_type and file_name may also be supplied.',
        },
      },
      required: ['mediaFiles'],
      additionalProperties: false,
  ...
  {
    name: 'github_get_branch_status',
    description: 'Return GitHub branch metadata.',
    inputSchema: {
      type: 'object',
  ...
  },
  {
    ...
    description: 'Create a GitHub branch from a commit SHA.',
    inputSchema: {
      type: 'object',
      properties: { repo: { type: 'string' }, branch: { type: 'string' }, sha: { type: 'string' } },
  ...
  {
    name: 'github_validate_change',
    description: 'Validate changed files against an allowlist and file-count limit.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Commit a prepared Git tree and advance a branch.',
    inputSchema: {
      type: 'object',
      properties: {
        repo: { type: 'string' },
      ...
      required: ['repo', 'branch', 'message', 'treeSha', 'parentSha'],
      additionalProperties: false,
    },
  ...
  },
  {
    ...
    description: 'Describe the safe GitHub fix workflow before executing it.',
    inputSchema: {
      type: 'object',
      properties: {
        repo: { type: 'string' },
      ...
      required: ['repo', 'branch', 'base', 'summary'],
      additionalProperties: false,
    },
    annotations: { readOnlyHint: true },
  },
...
function result(value: unknown): Obj {
  return {
    content: [{ type: 'text', text: JSON.stringify(value, null, 2) }],
    structuredContent: value,
  };
}
function required(args: Obj, key: string): string {
...
async function signed(value: string, secret: string): Promise<string> {
  return `${value}.${await signature(value, secret)}`;
}
async function verified(value: string | undefined, secret: string): Promise<string | null> {
  if (!value) return null;
  const split = value.lastIndexOf('.');
  if (split < 1) return null;
  const raw = value.slice(0, split),
    expected = await signature(raw, secret),
    actual = value.slice(split + 1);
...
function decodeCookiePayload<T>(value: string): T | null {
  try {
    const padded =
...
async function signedPayload<T>(value: T, secret: string): Promise<string> {
  return signed(encodeCookiePayload({ value, expiresAt: Date.now() + COOKIE_TTL * 1000 }), secret);
}
async function verifiedPayload<T>(cookie: string | undefined, secret: string): Promise<T | null> {
  const raw = await verified(cookie, secret);
  if (!raw) return null;
  const payload = decodeCookiePayload<{ value: T; expiresAt: number }>(raw);
  if (!payload || typeof payload.expiresAt !== 'number' || payload.expiresAt < Date.now())
    return null;
  return payload.value;
}
function setCookie(name: string, value: string, maxAge = COOKIE_TTL): string {
...
function clearAuthCookies(response: Response): Response {
  ...
  response.headers.append('Set-Cookie', clearCookie(GITHUB_STATE_COOKIE));
  response.headers.append('Set-Cookie', clearCookie('oauth_consent'));
  response.headers.append('Set-Cookie', clearCookie('github_state'));
  return response;
}
...
async function authDebug(env: WorkerEnv): Promise<Response> {
  const raw = await env.STATE.get(AUTH_DEBUG_KEY);
  return json({ ok: true, events: raw ? JSON.parse(raw) : [] }, 200, {
...
async function xrpc<T>(
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...(options.session ? { Authorization: `Bearer ${options.session.accessJwt}` } : {}),
      ...(options.body === undefined ? {} : { 'Content-Type': 'application/json' }),
    },
    body: options.body === undefined ? undefined : JSON.stringify(options.body),
  });
  if (!response.ok)
    throw new Error(`Bluesky API ${path} failed (${response.status}): ${await response.text()}`);
  return (await response.json()) as T;
}
async function session(env: WorkerEnv): Promise<Session> {
  return xrpc(env, 'com.atproto.server.createSession', {
    method: 'POST',
    body: { identifier: env.ATPROTO_IDENTIFIER, password: env.ATPROTO_PASSWORD },
  });
}
function postParts(uri: string): { repo: string; collection: string; rkey: string } {
...
async function cid(env: WorkerEnv, auth: Session, uri: string): Promise<string> {
  const record = await xrpc<{ cid?: string }>(env, 'com.atproto.repo.getRecord', {
    session: auth,
    query: new URLSearchParams(postParts(uri)),
  });
  if (!record.cid) throw new Error(`Could not resolve CID for ${uri}`);
  return record.cid;
}
async function activity(env: WorkerEnv, args: Obj): Promise<Obj> {
  const since =
    typeof args.since === 'string'
      ? args.since
      : ((await env.STATE.get(CHECKPOINT_KEY)) ?? undefined);
  const sinceMs = since ? Date.parse(since) : undefined;
  if (since && Number.isNaN(sinceMs)) throw new Error('since must be a valid date-time');
  const limit =
    typeof args.limit === 'number' ? Math.max(1, Math.min(100, Math.trunc(args.limit))) : 100;
  ...
  do {
    ...
    const response = await xrpc<{ notifications: Obj[]; cursor?: string }>(
      env,
    ...
    for (const item of response.notifications) {
      const indexedAt = typeof item.indexedAt === 'string' ? item.indexedAt : undefined;
      const indexedMs = indexedAt ? Date.parse(indexedAt) : NaN;
      if (sinceMs !== undefined && Number.isFinite(indexedMs) && indexedMs <= sinceMs) {
        reachedSince = true;
        continue;
      }
      if (typeof item.reason !== 'string' || !REASONS.has(item.reason)) continue;
      if (sinceMs !== undefined && (!Number.isFinite(indexedMs) || indexedMs <= sinceMs)) continue;
      items.push({
        id: `${String(item.uri)}:${String(item.cid)}:${String(item.reason)}`,
        kind: item.reason,
        uri: item.uri,
        cid: item.cid,
  ...
  } while (items.length < limit && cursor && !reachedSince && pagesScanned < 20);

  if (args.advanceCheckpoint === true) await env.STATE.put(CHECKPOINT_KEY, checkedAt);
  const activity = items.slice(0, limit);
  return {
    success: true,
    checkedAt,
    since: since ?? null,
    checkpointAdvanced: args.advanceCheckpoint === true,
    total: activity.length,
    counts: Object.fromEntries(
      [...REASONS].map(reason => [reason, activity.filter(item => item.kind === reason).length])
    ),
    activity,
    cursor: cursor ?? null,
    pagesScanned,
    truncated: Boolean(cursor && !reachedSince && pagesScanned >= 20),
  };
}
async function context(env: WorkerEnv, args: Obj): Promise<Obj> {
  const auth = await session(env);
  const query = new URLSearchParams({
    uri: required(args, 'uri'),
    depth: String(typeof args.depth === 'number' ? Math.trunc(args.depth) : 6),
    parentHeight: String(
      typeof args.parentHeight === 'number' ? Math.trunc(args.parentHeight) : 20
    ),
  });
  const response = await xrpc<Obj>(env, 'app.bsky.feed.getPostThread', { session: auth, query });
  return { success: true, thread: response.thread };
}
async function reply(env: WorkerEnv, args: Obj): Promise<Obj> {
  const text = required(args, 'text'),
    root = required(args, 'root'),
    parent = required(args, 'parent'),
    auth = await session(env);
  const [rootCid, parentCid] = await Promise.all([cid(env, auth, root), cid(env, auth, parent)]);
  const record: Obj = {
    $type: 'app.bsky.feed.post',
    text,
    createdAt: new Date().toISOString(),
    reply: { root: { uri: root, cid: rootCid }, parent: { uri: parent, cid: parentCid } },
  };
  if (Array.isArray(args.langs))
    record.langs = args.langs.filter(value => typeof value === 'string');
...
async function uploadImageBlob(env: WorkerEnv, auth: Session, input: unknown): Promise<Obj> {
  ...
  if ((urlValue ? 1 : 0) + (base64Value ? 1 : 0) !== 1)
    throw new Error('Each image must provide exactly one of url or base64');
  ...
  if (bytes.byteLength > IMAGE_MAX_SIZE)
    throw new Error('Each Bluesky image must be 2 MB or smaller');

  const uploadUrl = new URL(
    '/xrpc/com.atproto.repo.uploadBlob',
    env.ATPROTO_SERVICE || 'https://bsky.social'
  );
  ...
  if (!response.ok)
    throw new Error(`Bluesky image upload failed (${response.status}): ${await response.text()}`);
...
function pdsAudienceFromAccessJwt(accessJwt: string): string {
  const payloadPart = accessJwt.split('.')[1];
  const payload = payloadPart ? decodeCookiePayload<unknown>(payloadPart) : null;
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
    throw new Error('Could not determine the PDS service DID from the Bluesky session token');
  }

  const aud = (payload as Obj).aud;
  const audience =
    typeof aud === 'string'
      ? aud
  ...
  if (!audience || (!audience.startsWith('did:plc:') && !audience.startsWith('did:web:'))) {
    throw new Error('Bluesky session token does not contain a supported PDS service DID audience');
  }
  return audience;
}

async function getVideoServiceAuth(env: WorkerEnv, auth: Session): Promise<string> {
  // Legacy createSession access tokens are issued to the PDS service DID in
  // their aud claim. Use that authoritative session value instead of trying
  // to resolve the account DID through the PDS itself: a PDS is not required
  // to implement com.atproto.identity.resolveDid, and its hostname does not
  // need to be converted into a guessed did:web service identity.
  const query = new URLSearchParams({
    aud: pdsAudienceFromAccessJwt(auth.accessJwt),
    lxm: 'com.atproto.repo.uploadBlob',
    exp: String(Math.floor(Date.now() / 1000) + 60 * 30),
  });
  const response = await xrpc<{ token?: string }>(env, 'com.atproto.server.getServiceAuth', {
    session: auth,
    query,
  });
...
function normalizeVideoJob(value: unknown): Obj {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error('Invalid response from Bluesky video service');
...
async function uploadVideoEmbed(env: WorkerEnv, auth: Session, input: unknown): Promise<Obj> {
  ...
  const mimeType =
    typeof item.mimeType === 'string' && item.mimeType ? item.mimeType : detectedMime;
  if (!VIDEO_MIME_TYPES.has(mimeType))
    throw new Error(`Unsupported Bluesky video type: ${mimeType || 'unknown'}`);
  ...
  if (!upload.ok)
    throw new Error(`Bluesky video upload failed (${upload.status}): ${await upload.text()}`);
  ...
  for (let attempt = 0; attempt < 75 && !job.blob; attempt++) {
    ...
    if (/failed|aborted|expired/i.test(state))
      throw new Error(
        `Bluesky video processing failed: ${String(job.error ?? job.message ?? state)}`
      );
    if (!jobId) break;
    await sleep(1000);
    ...
    if (!status.ok)
      throw new Error(
        `Bluesky video status check failed (${status.status}): ${await status.text()}`
      );
...
async function createPost(env: WorkerEnv, args: Obj): Promise<Obj> {
  ...
  if (mediaFiles.length) {
    if (images.length || videoInput !== undefined)
      throw new Error('Use either mediaFiles or explicit images/video, not both');
    const detected = await Promise.all(
      mediaFiles.map(async input => {
        if (typeof input === 'string') {
          const url = input;
          return { url, mimeType: await detectRemoteMime(url) };
        }
        if (!input || typeof input !== 'object' || Array.isArray(input))
          throw new Error('Each mediaFiles item must be a ChatGPT file object');
        ...
        const name =
          typeof file.file_name === 'string' && file.file_name ? file.file_name : undefined;
        return { url, mimeType, ...(name ? { name } : {}) };
      })
    );
    const videos = detected.filter(item => VIDEO_MIME_TYPES.has(item.mimeType));
    const stills = detected.filter(
      item => item.mimeType.startsWith('image/') && item.mimeType !== 'image/gif'
    );
    if (videos.length) {
      if (detected.length !== 1 || videos.length !== 1)
        throw new Error('A Bluesky video post can contain one video and no images');
      videoInput = videos[0];
    } else {
  ...
  if (images.length && hasVideo)
    throw new Error('A Bluesky post cannot contain both image and video embeds');
...
async function invoke(env: WorkerEnv, name: unknown, args: Obj): Promise<Obj> {
  ...
  if (name === 'record_devlog_post')
    return result(
      await record(
        env,
        required(args, 'uri'),
        Array.isArray(args.commitReferences)
          ? args.commitReferences.filter((r): r is string => typeof r === 'string')
          : []
      )
    );
  if (name === 'get_community_activity') return result(await activity(env, args));
  if (name === 'get_post_context') return result(await context(env, args));
  ...
  if (name === 'github_create_branch') return result(await githubCreateBranch(env, args));

  if (name === 'github_apply_patch') return result(await githubApplyPatch(env, args));

  if (name === 'github_validate_change') return result(await githubValidateChange(env, args));

  if (name === 'github_commit_changes') return result(await githubCommitChanges(env, args));
...
async function mcp(request: Request, env: WorkerEnv): Promise<Response> {
  if (request.method !== 'POST') return new Response('Method not allowed', { status: 405 });
  const message = (await request.json()) as Obj,
    id = message.id ?? null;
  ...
  if (message.method === 'tools/call') {
    ...
    try {
      return rpc(id, await invoke(env, params.name, (params.arguments ?? {}) as Obj));
    } catch (error) {
      return rpc(id, {
        isError: true,
...
async function authorize(request: Request, env: WorkerEnv): Promise<Response> {
  if (request.method === 'GET') {
    ...
    const response = consentPage(
      String((client as unknown as Obj).clientName ?? oauthRequest.clientId),
      csrf
    );
    response.headers.append('Set-Cookie', clearCookie(CONSENT_COOKIE));
    response.headers.append('Set-Cookie', clearCookie(GITHUB_STATE_COOKIE));
    ...
    response.headers.append(
      ...
      setCookie(
        CONSENT_COOKIE,
        await signedPayload({ oauthRequest, csrf, githubState }, env.COOKIE_ENCRYPTION_KEY)
      )
    );
    return response;
  }
  if (request.method !== 'POST') return new Response('Method not allowed', { status: 405 });
  await recordAuthStage(env, 'authorize_post');
  const form = await request.formData();
  const pending = await verifiedPayload<{
    oauthRequest: AuthRequest;
    csrf: string;
    githubState: string;
  }>(cookies(request)[CONSENT_COOKIE], env.COOKIE_ENCRYPTION_KEY);
  if (!pending) return new Response('Authorization session is invalid or expired', { status: 400 });
  if (form.get('csrf') !== pending.csrf)
    return new Response('CSRF validation failed', { status: 403 });
  if (form.get('decision') !== 'approve') {
    const denied = new URL(pending.oauthRequest.redirectUri);
    denied.searchParams.set('error', 'access_denied');
    denied.searchParams.set('state', pending.oauthRequest.state);
    return clearAuthCookies(Response.redirect(denied.toString(), 302));
  }
  const state = pending.githubState;
  const redirect = new URL('https://github.com/login/oauth/authorize');
  redirect.searchParams.set('client_id', env.GITHUB_APP_CLIENT_ID);
  redirect.searchParams.set('redirect_uri', `${new URL(request.url).origin}/callback`);
  redirect.searchParams.set('state', state);
  await recordAuthStage(env, 'redirect_to_github');
  const response = new Response(null, { status: 302, headers: { Location: redirect.toString() } });
  response.headers.append(
    'Set-Cookie',
    setCookie(
      GITHUB_STATE_COOKIE,
      await signedPayload({ state, oauthRequest: pending.oauthRequest }, env.COOKIE_ENCRYPTION_KEY)
    )
  );
  return response;
}
async function callback(request: Request, env: WorkerEnv): Promise<Response> {
  await recordAuthStage(env, 'github_callback');
  const url = new URL(request.url),
    code = url.searchParams.get('code'),
    state = url.searchParams.get('state'),
    pending = await verifiedPayload<{ state: string; oauthRequest: AuthRequest }>(
      cookies(request)[GITHUB_STATE_COOKIE],
      env.COOKIE_ENCRYPTION_KEY
    );
  if (!code || !state || !pending || state !== pending.state)
    return new Response('GitHub OAuth state validation failed', { status: 400 });
  await recordAuthStage(env, 'github_state_ok');
  ...
  if (!tokenResponse.ok || !tokenData.access_token)
    return new Response(
      `GitHub token exchange failed: ${tokenData.error_description ?? tokenResponse.status}`,
      { status: 502 }
    );
  await recordAuthStage(env, 'github_token_ok');
  ...
  if (!userResponse.ok || !user.id || !user.login)
    return new Response('Could not verify GitHub identity', { status: 502 });
  if (user.login.toLowerCase() !== env.GITHUB_ALLOWED_LOGIN.toLowerCase())
    return new Response('This GitHub account is not authorized for this Bluesky manager', {
      status: 403,
    });
  await recordAuthStage(env, 'github_user_ok');
  const completed = await env.OAUTH_PROVIDER.completeAuthorization({
    request: pending.oauthRequest,
    userId: `github:${user.id}`,
    metadata: { label: user.login },
    scope: pending.oauthRequest.scope,
    props: { githubId: user.id, githubLogin: user.login },
  });
  await recordAuthStage(env, 'authorization_completed');
  const response = new Response(null, { status: 302, headers: { Location: completed.redirectTo } });
  response.headers.append('Set-Cookie', setCookie('github_state', '', 0));
  return response;
}
