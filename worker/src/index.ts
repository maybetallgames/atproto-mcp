...
type WorkerEnv = {
  ...
  GITHUB_APP_ID: string; GITHUB_PRIVATE_KEY: string; GITHUB_INSTALLATION_ID: string;
  COOKIE_ENCRYPTION_KEY: string; MCP_SECRET_PATH: string; STATE: KV; OAUTH_KV: KV; OAUTH_PROVIDER: OAuthHelpers;
};

const tools = [
  { name: 'github_get_recent_commits', description: 'Read recent Lets Dive commits.', inputSchema: { type: 'object', properties: { since: { type: 'string', format: 'date-time' } }, additionalProperties: false }, annotations: { readOnlyHint: true } },
  { name: 'github_get_commit_details', description: 'Read changed files for a Lets Dive commit.', inputSchema: { type: 'object', properties: { sha: { type: 'string' } }, required: ['sha'], additionalProperties: false }, annotations: { readOnlyHint: true } },
  { name: 'github_get_recent_prs', description: 'Read recently merged Lets Dive pull requests.', inputSchema: { type: 'object', properties: { since: { type: 'string', format: 'date-time' } }, additionalProperties: false }, annotations: { readOnlyHint: true } },
  { name: 'create_devlog_update', description: 'Read and summarize new development facts for the social layer. Does not publish or advance the checkpoint.', inputSchema: { type: 'object', properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true } },
  { name: 'record_devlog_post', description: 'After a successful Bluesky devlog post, record its URI and commit references in Cloudflare KV.', inputSchema: { type: 'object', properties: { uri: { type: 'string' }, commitReferences: { type: 'array', items: { type: 'string' }, minItems: 1 } }, required: ['uri', 'commitReferences'], additionalProperties: false }, annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true } },
...
async function activity(env: WorkerEnv, args: Obj): Promise<Obj> {
  ...
  return {
    ...
    counts: Object.fromEntries([...REASONS].map(reason => [reason, activity.filter(item => item.kind === reason).length])),
    activity,
    cursor: cursor ?? null,
    pagesScanned,
